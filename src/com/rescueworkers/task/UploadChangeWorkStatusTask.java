...
public class UploadChangeWorkStatusTask extends BaseTask{
	private String token;
	private Context context;
	private Task task;
	public UploadChangeWorkStatusTask(String preDialogMessage, Context context,Task task) {
	...
	public JsonPack getData() throws Exception {
		if (!Settings.DEBUG) {
			String work_jobs = TaskDb.getUnUploadTask("0");
			JSONObject jsonObj = new JSONObject();
			jsonObj.put("work_jobs", work_jobs);
            // 从任务状态切换列表中，获取最多10条信息，组装成一条数据上传
			return A57HttpApiV3.getInstance().postRescueWorkStatusChange(token, jsonObj.toString());
